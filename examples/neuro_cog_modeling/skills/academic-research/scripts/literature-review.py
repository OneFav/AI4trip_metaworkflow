...
Built by Topanga (topanga.ludwitt.com) 鈥?AI Research Consultant
"""
import argparse
import hashlib
import json
...
CACHE_DIR = Path(os.getenv("TMP", ".")) / "litreview_cache"
...
def generate_synthesis_md(topic, papers, themes, year_range=None):
    ...
    for i, p in enumerate(top, 1):
        ...
        doi = f" 鈥?DOI: {p['doi']}" if p.get("doi") else ""
        lines.append(f"{i}. **{p['title']}** 鈥?{authors}{yr} [{p.get('citations', 0)} citations]{doi}")
    ...
    for theme, tpapers in themes.items():
        ...
        for p in tpapers[:5]:
            ...
            if len(p.get("authors", [])) > 2:
                authors += " et al."
            yr = f" ({p['year']})" if p.get("year") else ""
            oa = "馃敁" if p.get("open_access") else "馃敀"
            lines.append(f"- {oa} **{p['title']}** 鈥?{authors}{yr}")
            if p.get("abstract"):
...
def main():
    ...
    if args.output:
        ...
        print(f"\n鉁?Review written to {args.output}", file=sys.stderr)
    else:
        print(output)

    print(f"\n馃搳 Summary: {len(papers)} papers, {len(themes)} themes", file=sys.stderr)
