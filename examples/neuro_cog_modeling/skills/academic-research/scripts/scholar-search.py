...
def main():
    ...
    dp = sub.add_parser("deep", help="Deep read 鈥?full details + PDF URL")
    dp.add_argument("doi")
    ...
    if args.cmd == "search":
        ...
        if use_json:
            ...
            print(f"馃攳 Found {len(results)} results for: {args.query}\n")
        ...
        if use_json:
            ...
            for dir_name, papers in result.items():
                print(f"\n{'馃摜' if dir_name == 'cited_by' else '馃摛'} {dir_name.replace('_', ' ').title()} ({len(papers)}):\n")
